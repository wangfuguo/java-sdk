...
import com.zeyiyouhuo.api.FileItem;
...
public abstract class WebUtils {
     ...
     * @return 响应字符串
     */
    public static HttpResponseData doGet(String url, Map<String, String> headerMap, String charset, int connectTimeout, int readTimeout, Proxy proxy) throws IOException {
        HttpURLConnection conn = null;
        String rsp = null;
        HttpResponseData data = new HttpResponseData();
        try {
            String ctype = "application/x-www-form-urlencoded;charset=" + charset;
            conn = getConnection(buildGetUrl(url, null), Constants.METHOD_GET, ctype, headerMap, proxy);
            conn.setConnectTimeout(connectTimeout);
            conn.setReadTimeout(readTimeout);
            rsp = getResponseAsString(conn);
            data.setBody(rsp);
    ...
    private static HttpURLConnection getConnection(URL url, String method, String ctype, Map<String, String> headerMap, Proxy proxy) throws IOException {
        ...
        if (headerMap != null && headerMap.get(Constants.HTTP_DNS_HOST) != null) {
            conn.setRequestProperty("Host", headerMap.get(Constants.HTTP_DNS_HOST));
        } else {
            conn.setRequestProperty("Host", url.getHost());
        }
        conn.setRequestProperty("Accept", "text/xml,text/javascript");
        conn.setRequestProperty("User-Agent", "Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/92.0.4515.159 Safari/537.36");
        conn.setRequestProperty("Content-Type", ctype);
        if (headerMap != null) {
            for (Map.Entry<String, String> entry : headerMap.entrySet()) {
                if (!Constants.HTTP_DNS_HOST.equals(entry.getKey())) {
    ...
    public static String buildQuery(Map<String, String> params, String charset) throws IOException {
        ...
        for (Map.Entry<String, String> entry : entries) {
            ...
            // 忽略参数名或参数值为空的参数
            if (StringUtils.allNotEmpty(name, value)) {
                if (hasParam) {
                    query.append("&");
                } else {
                    hasParam = true;
